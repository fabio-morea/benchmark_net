...
df <- data.frame(mu = numeric(), NMI = numeric())
...
for (mui in seq(5, 95, 5)) {
	...
	ccs <- compare_clustering_results(all_clusters, 
						threshold = .5, 	 # proportion of membership to be assigned to community 0
						min_vids = 1, 		 # number of vertices to be assigned to community 0
						min_weight = 1/100000)# fraction of whole network weight to be assigned to community 0
	...
	louvain_labels <- V(g)$comm_louvain
	#print(louvain_labels)
	mut_inf <- mutinformation(true_labels,louvain_labels, method="emp")
	entr    <- sqrt(entropy(true_labels) * entropy(louvain_labels) )
	NMI     <- round(mut_inf/ entr,3)
	print(paste("Normalized Mutual Information",NMI))

	 df <- rbind(df, c(mui, NMI))
	
}

df <- rename(df, c("X5" = "mu", "X1" = "NMI"))
write.csv(df, file = "df.csv")




ggplot(df, aes(mu, NMI)) +   geom_line() + geom_point(shape = 4)
